...
export default function ExpenseTracker() {
  ...
  // Hook A: Instant Global Authentication Check
  useEffect(() => {
    const token = localStorage.getItem("accessToken");
    if (!token) {
  ...
  // Hook B: Seed data only if projectId exists and user is authenticated
  useEffect(() => {
    if (projectId && localStorage.getItem("accessToken")) {
      fetchMetaOptions();
      fetchExpenses();
    }
  }, [projectId]);

  // Hook C: Quick fetch whenever filters change
  useEffect(() => {
    if (projectId && localStorage.getItem("accessToken")) {
      fetchExpenses();
    }
